...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        track = [{"index": 0, "label": "mono", "source_channel": args.channel,
                  "channel_mode": args.channel, "result": result, "profile": profile}]
        out = pipeline.export(track, adr, settings, Path(args.out))
        if args.export_ir:
            ir_out = pipeline.export(track, adr, settings, Path(args.out), mode="ir_profile")
